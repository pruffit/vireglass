...
// into the other. Pure functions, no imports: the physics is checked by tests, not on a device.
//
// There are normalization constants here, and they're honestly labeled as such: the rendering is
// stylized, and literal physical values (glass reflects 4% at normal incidence) produce effects
// right at the threshold of visibility. Each normalization is monotonic in its own cause, so
// "denser medium → more visible rim" always holds, and no impossible combination can arise.
...
/** Translation of the physical 4% into the render's working range. */
...
/** Magnification behind a plane-parallel plate: grows with thickness and with (1 − 1/n). */
...
/** In ordinary glasses the Abbe number falls as n rises — dispersion grows along with it. */
const DISPERSION_PER_IOR = 1.1;

export const dispersion = (ior: number) => clamp01((ior - 1) * DISPERSION_PER_IOR);

/** Spread of the index between the red and blue channels. The physical value (~0.01 at an Abbe
 *  number of 55) sits at the threshold of visibility — a stylization of the same monotonicity as
 *  Fresnel's. */
const IOR_SPREAD_PER_DISPERSION = 0.045;

export const iorSpread = (ior: number) => dispersion(ior) * IOR_SPREAD_PER_DISPERSION;

/** Beer–Lambert: how much the medium absorbs over a path of length `pathDp`. */
