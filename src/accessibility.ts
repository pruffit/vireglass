// System accessibility settings change the material's LAYERS, they don't override it (reference
// 219 @18:15). So what's adjusted here are EFFECTS, like with toggles: the shader knows nothing
// about accessibility.
import type { VireGlassOptics } from './material';

export type VireGlassAccessibility = {
  /** The glass becomes more frosted and hides content beneath it more strongly (219 @18:22). */
  reduceTransparency: boolean;
  /** The element moves nearly to black or white and picks up a contrasting border (219 @18:29). */
  increaseContrast: boolean;
  /** The material's springiness is turned off, effects are quieter (219 @18:35). Doesn't touch
   *  optics: this is about motion, applied by whoever computes that motion. */
  reduceMotion: boolean;
};

export const NO_ACCESSIBILITY: VireGlassAccessibility = {
  reduceTransparency: false,
  increaseContrast: false,
  reduceMotion: false,
};

/** Frost and density below which "reduced transparency" glass never drops. */
const FROST_MIN_DP = 14;
const OBSCURE_MIN = 0.45;
/** Contrast mode: the body is nearly opaque, and the silhouette must be fully visible. */
const CONTRAST_DENSITY = 0.9;
const CONTRAST_PRESENCE = 0.5;

/** A system setting acts on the whole glass and outranks the material preset (219 @18:45): even
 *  a transparent preset moves to the edge of the scale under contrast — the user needs contrast
 *  more than they need aesthetics. */
export function applyAccessibility(
  optics: VireGlassOptics,
  mods: VireGlassAccessibility = NO_ACCESSIBILITY,
): VireGlassOptics {
  if (!mods.reduceTransparency && !mods.increaseContrast) return optics;
  const out = { ...optics };
  if (mods.reduceTransparency) {
    out.blur = Math.max(out.blur, FROST_MIN_DP);
    out.bodyDensity = Math.max(out.bodyDensity, OBSCURE_MIN);
  }
  if (mods.increaseContrast) {
    // The border is held by silhouette presence (`presence`), not by filling the body: filling
    // the body dims the content, and the reference's requirement is specifically a contrasting
    // outline around the element.
    out.bodyDensity = Math.max(out.bodyDensity, CONTRAST_DENSITY);
    out.presence = Math.max(out.presence, CONTRAST_PRESENCE);
    out.legibility = 1;
  }
  return out;
}
